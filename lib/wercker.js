...
Wercker.prototype.create = function(environment) {
  ...
  async.waterfall([
    ...
    function(result, callback){
      if(!result.success){
        if(result.data && result.data.authenticateUrl){
          console.log(util.format('Please goto %s to connect to this git provider.', result.data.authenticateUrl));
        }

        else{
          console.log('Cannot create project: ' + result.errorMessage);
        }
      }
      else{
